...
//    printf("** Checking threshold..\r\n");
    ...
    while (startIndex < stopIndex){
        if(half_buffer[startIndex] >= UPPER_THRESHOLD && FingerPositions[0] > MIN_FINGER_POSITION){ // Update to check more than thumb
        ...
        } else if(half_buffer[startIndex] < LOWER_THRESHOLD && FingerPositions[0] < MAX_FINGER_POSITION){
            Hand_Move(FingerPositions[0] - 1, allFingers);
